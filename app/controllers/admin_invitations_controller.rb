#   Copyright (c) 2010, Diaspora Inc.  This file is
#   licensed under the Affero General Public License version 3 or later.  See
#   the COPYRIGHT file.

class AdminInvitationsController < Devise::InvitationsController

  before_filter :check_token, :only => [:edit]

  def create
    begin
      #params[:user][:aspect_id] = params[:user].delete(:aspects)
      message = params[:admin].delete(:invite_messages)
      params[:admin][:invite_message] = message unless message == ""
      self.resource = current_admin.invite_admin(params[resource_name])
      flash[:notice] = I18n.t 'invitations.create.sent'
    rescue RuntimeError => e
      if  e.message == "You have no invites"
        flash[:error] = I18n.t 'invitations.create.no_more'
      elsif e.message == "You already invited this person"
        flash[:error] = I18n.t 'invitations.create.already_sent'
      elsif e.message == "You are already friends with this person"
        flash[:error] = I18n.t 'invitations.create.already_friends'
      else
        raise e
      end
    end
    redirect_to after_sign_in_path_for(resource_name)
    #redirect_to admin_root_path
  end
  
  #def update
  #  begin
  #    admin = Admin.find_by_invitation_token(params["admin"]["invitation_token"])
  #    admin.accept_invitation!(params["admin"])
  #     rescue MongoMapper::DocumentNotValid => e
  #      admin = nil
  #    flash[:error] = e.message
  #  end
   # if admin
  #     flash[:notice] = I18n.t 'registrations.create.success'
  #    sign_in_and_redirect(:admin, admin)
  #    else
  #    redirect_to new_user_registration_path
  #  end
 # end

  protected

  def check_token
    if Admin.find_by_invitation_token(params['invitation_token']).nil?
      flash[:error] = "Invitation token not found"
      redirect_to admin_root_path
    end
  end
end
